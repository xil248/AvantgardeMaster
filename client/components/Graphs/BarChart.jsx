...
export default class BarChart extends React.Component {
  ...
  createSvgRoot({ node, m }) {
    return select(node)
      .append('g')
      .on('mouseleave', this.props.mouseOutHandler)
  ...
  createXAxis({ root, m, w, h, x }) {
    ...
    const group = root
      .append('g')
      .attr('class', 'x axis')
      .attr('transform', `translate(${w / this.props.data.length / 2}, ${h})`);

    group
      .call(axis);

    // This is to rotate the labels for graphs and push them down
    // a little bit so that they do not land above the tick.
    group
      .selectAll('.tick text')
      .attr('transform', 'translate(-20, 20), rotate(-50)');
      ...
      group
        ...
        .attr('x',
          (yAxisOrientRight)
            ? 0
            : w)
        // push the labels 5 pixels up so that they are not cut off
        // the bottom as well as not get in the way of tick labels
